...
router.post('/avatar', upload.single('avatar'), async (req, res) => {
    try {
        ...
        const filename = `avatar-${userId}-${(0, uuid_1.v4)()}.jpg`;
        ...
        logger_1.default.info(`Avatar uploaded for user: ${req.user.username}`);
        res.json({
...
router.post('/file', upload.single('file'), async (req, res) => {
    try {
        ...
        logger_1.default.info(`File uploaded by user ${req.user.username}: ${originalName}`);
        res.json({
...
router.post('/files', upload.array('files', 5), async (req, res) => {
    try {
        ...
        logger_1.default.info(`${files.length} files uploaded by user ${req.user.username}`);
        res.json({
...
router.delete('/file/:filename', async (req, res) => {
    try {
        ...
        logger_1.default.info(`File deleted by user ${req.user.username}: ${filename}`);
        res.json({

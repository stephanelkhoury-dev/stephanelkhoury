      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12">
        <div className="mb-16 flex flex-col md:flex-row md:items-end justify-between gap-6">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="max-w-2xl">
            ...
            <p className="text-zinc-400 text-lg">A selection of digital platforms, applications, and tools engineered for performance and designed for users.</p>
          </motion.div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {projects.map((project, index) => (
            <motion.article
              ...
              className="group bg-zinc-900/50 rounded-3xl overflow-hidden border border-zinc-800/80 hover:border-zinc-700 transition-all flex flex-col"
            >
              ...
              <div className="p-8 flex-1 flex flex-col">
                <div className="flex justify-between items-start mb-4">
                  <h4 className="text-2xl font-bold text-zinc-100 group-hover:text-blue-400 transition-colors">{project.title}</h4>
                  <Link href={`/projects/${project.slug}`} className="p-2 rounded-full bg-zinc-800 text-zinc-400 group-hover:bg-blue-600 group-hover:text-white transition-colors">
                ...
                <div className="flex flex-wrap gap-2 mt-auto pt-4 border-t border-zinc-800/50">
                  {project.technologies.map((tag) => (
                    <span key={`${project.id}-${tag}`} className="text-xs font-medium text-zinc-500 bg-zinc-900 px-2 py-1 rounded-md">
                      {tag}
                    </span>
                  ))}
                </div>
